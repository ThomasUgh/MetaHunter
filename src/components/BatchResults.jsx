...
function BatchResults({ results }) {
  ...
  const exportAll = () => {
    ...
    a.href = url
    a.download = 'batch_metadata.json'
      ...
      <div className="bg-[var(--bg-secondary)] rounded-xl px-5 py-4 border border-[var(--border)] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{t('batch.filesProcessed', { count: results.length })}</p>
          </div>
        </div>
        
        <button
          onClick={exportAll}
          className="px-4 py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/30 hover:bg-cyan-500/20 transition-colors text-cyan-400 text-sm"
        >
          {t('export.exportAll')}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="md:col-span-1 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] overflow-hidden">
          <div className="px-4 py-3 bg-[var(--bg-tertiary)] border-b border-[var(--border)]">
            <h3 className="text-sm font-medium text-gray-400">{t('batch.files')}</h3>
          </div>
        ...
        <div className="md:col-span-3 space-y-4">
          <div className="flex justify-end">
            <ExportBar data={selected.data} fileName={selected.name} />
          </div>
