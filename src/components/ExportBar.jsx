...
function ExportBar({ data, fileName }) {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      {buttons.map(btn => (
        <button key={btn.label} onClick={btn.fn} className="px-3 py-1.5 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] hover:border-cyan-500/50 hover:bg-cyan-500/10 transition-all text-gray-300 flex items-center gap-1.5">
          <span className="text-xs">{btn.icon}</span><span>{btn.label}</span>
        </button>
      ))}
    </div>
  )
