  ...
  return (
    <header className="mb-10">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          onClick={toggleLang}
          className="px-3 py-1.5 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border)] hover:border-cyan-500/50 transition-colors text-sm flex items-center gap-2"
        >
          <span>{lang === 'en' ? '🇬🇧' : '🇩🇪'}</span>
      ...
      <div className="inline-flex items-center gap-2 text-xs">
        ...
        <span className="text-gray-500">{t('upload.supportedFormats')}</span>
      </div>
    </header>
  )
}
